...
void Set_Font_8_by_8()
{
	GlyphSetFont(lcd_handle, GLYPH_FONT_8_BY_8);	
}	

void Set_Font_Bitmap()
{
...
void Clear_Screen(void)
{
	...
	for(i = 2; i <= (2+(96*8)); i++)
	{
		Screen[i] = Screen[i] & 0x00; 
	}
}
void LCD_Set_Pixel(int x, int y)
{
	int xIndex = 0, yIndex =0, shiftby = 0;
	if(x >= 96 || y >= 64 || x < 0 || y < 0) 
	return ;
	else
	{
		yIndex = y/8;
		shiftby = y - (yIndex*8);
		xIndex = 2 + (yIndex*96) + x;
		Screen[xIndex] = Screen[xIndex] | (1 << shiftby);
	}
}
